...
local function create_calendar()
   ...
   local result = "do lu ma me ju ve sa\n" -- days of the week
   ...
   for day = 1, last_day do
      ...
      if cal_month == now.month and cal_year == now.year and day == now.day then
         this_month = true
         result = result ..
            string.format('<span weight="bold" foreground ="%s">%s</span>', today_color, day_str)
      else
   ...
   return header, string.format('<span font="%s" foreground="%s">%s</span>', theme.font, text_color, result)
end
